...
export class Socket {
  ...
  sendHeartbeat(){ if(!this.isConnected()){ return }
    this.push({channel: "socket", event: "heartbeat"})
  }
...
function getSingleValue(data, index) {
  var dv = new DataView(data.slice(index, index + 4))
  return [dv.getFloat32(0, false)]
}

function getSingleFloat64(data, index) {
  var dv = new DataView(data.slice(index, index + 8))
...
function roundNumbers(array, precision) {
  let multiplier = 10 * precision
  return _.map(array, num => {
    return Math.round(num * multiplier) / multiplier
  })
}

function getArrayValues(data, start_index, end_index, length) {
  var dv = new DataView(data.slice(start_index, end_index))
  var values = []
